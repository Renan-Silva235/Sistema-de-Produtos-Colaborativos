...
def pegar_dados_alimenticios():
    ...
    if not Validacoes.validar_campos(produto, origem):
...
def pegar_dados_vestuario():
    ...
    if not Validacoes.validar_campos(tipo_roupa, marca, cor, origem):
        print("Nenhum campo pode estar vazio")
        time.sleep(2)
        limpar_tela()
        return
...
def pegar_dados_domestico():
    ...
    if not Validacoes.validar_campos(produto, cor, quantidade, origem):
        print("Nenhum campo pode estar vazio")
        time.sleep(2)
        limpar_tela()
        return
